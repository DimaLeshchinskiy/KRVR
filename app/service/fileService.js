...
const rasterGraphicService = require('../service/RasterGraphicService');
...
const rasterGraphic2c = require("../convert/RasterGraphic2CANVAS");

//this generate gcode
const dxf2g = require("../convert/DXF2GC");
/*const png2g = require("../convert/PNG2GC");
const bmp2g = require("../convert/BMP2GC");*/
const rasterGraphic2g = require("../convert/RasterGraphic2GC");
...
const extensions2D = [".dxf", ".gcode"];
const rasterGraphicExtensions = [".png", ".bmp", ".jpg"];
...
function getAllExtensions(){
  return extensions2D.concat(extensions3D).concat(rasterGraphicExtensions);
}
...
function isFileRasterGraphic(file){
  return rasterGraphicExtensions.includes("." + file.extension);
}
...
exports.isFileRasterGraphic = isFileRasterGraphic;
...
exports.getCanvas = function(file){
  ...
  if(isFileRasterGraphic(file))
    return rasterGraphic2c.getCanvas(file);
...
*/
  if(isFileRasterGraphic(file))
    return rasterGraphic2g.getGcode(file);
...
exports.getFile = async function(path){
  ...
  if(rasterGraphicExtensions.includes(extension)){
    params.autoload = false;
    file = await rasterGraphicService.getRasterGraphic(params);
  }
  if(extension == ".gcode"){
    params.encoding = "utf-8";
